...
async def mock_ollama_get(path,timeout=10.0):
    if"/api/tags"in path:return{"models":[{"name":"qwen3:8b","size":2e9},{"name":"nomic-embed-text:latest","size":5e8}]}
    return{}
...
async def test_models(client):
    r=await client.get("/api/models");assert "qwen3:8b" in [m["name"] for m in r.json()["models"]]
...
async def test_query_returns_immediately(client):
    ...
    r=await client.post("/api/query",json={"query":"What is 42?","model":"qwen3:8b"})
...
async def test_query_answer_not_ready(client):
    ...
    r=await client.post("/api/query",json={"query":"Test","model":"qwen3:8b"})
    qid=r.json()["query_id"]
...
async def test_full_flow(client):
    ...
    r1=await client.post("/api/query",json={"query":"Hi?","model":"qwen3:8b"})
    ...
    r2=await client.post("/api/query",json={"query":"What is Python?","model":"qwen3:8b"})
